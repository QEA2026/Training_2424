-- Week 4 Monday — Instructor demo: DML with disciplined WHERE clauses (PostgreSQL)
-- Prerequisites: run code/demo_ddl_schema.sql first
-- Theory tie-in: written/dml-data-manipulation-language.md

BEGIN;

-- Clean slate for repeatable demo (optional)
...
-- DELETE: dangerous without WHERE — show safe pattern
DELETE FROM order_line
WHERE order_id = (
    SELECT order_id FROM order_header oh
    JOIN customer c ON c.customer_id = oh.customer_id
...
-- For demo, cancel is a no-op unless you set status — illustrate check first:
